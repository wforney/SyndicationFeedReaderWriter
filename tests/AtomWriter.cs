...
public class AtomWriter
{
    ...
    public async Task EmbededAtomInRssFeed()
    {
        ...
        using (var xmlWriter = XmlWriter.Create(sw))
        {
            ...
            var writer = new Rss.RssFeedWriter(xmlWriter, attributes);
            var formatter = new AtomFormatter(attributes, xmlWriter.Settings);

            // Write Rss elements
            await writer.WriteValue(Rss.RssElementNames.Title, "Rss Title");
        ...
        Assert.Contains($"<atom:entry><atom:id>{entry.Id}</atom:id><atom:title>{entry.Title}</atom:title><atom:updated>{entry.LastUpdated.ToRfc3339()}</atom:updated><atom:author><atom:name>{author.Name}</atom:name><atom:email>{author.Email}</atom:email></atom:author><atom:content>{entry.Description}</atom:content></atom:entry>", res);
    }
    ...
    private static bool CheckResult(string result, string expected) => result == $"<?xml version=\"1.0\" encoding=\"utf-8\"?><feed xmlns=\"http://www.w3.org/2005/Atom\">{expected}</feed>";

    private static bool CheckResult(string result, string expected, string prefix) => result == $"<?xml version=\"1.0\" encoding=\"utf-8\"?><feed xmlns:{prefix}=\"http://www.w3.org/2005/Atom\">{expected}</feed>";
}
...
internal sealed class StringWriterWithEncoding(Encoding encoding) : StringWriter
{
